...
if (!class_exists('ThemeToolkit')) {
	class ThemeToolkit{
		...
		function admin_menu () {
			...
			<table cellspacing="20" cellpadding="0" border="0" width=100% class="editform">';
			...
			foreach ($this->infos['menu_options'] as $key=>$val) {
				...
				print "<tr valign='top'><th scope='row' width='20%'>\n";
				if (@$items) {
					...
					print "<label for='$key'>".$matches[1]."</label></th>\n<td>";
					print "<input type='text' style=width:550px name='$key' id='$key' value='" . $this->option[$key] . "' />";
				}
