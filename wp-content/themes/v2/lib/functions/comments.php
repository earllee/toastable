...
function thesis_comments_link() {
	...
	if (!is_single() && !is_page()) { #wp
		if (comments_open() || (!comments_open() && $thesis_design->display['comments']['show_closed'])) {
			...
			$link = (comments_open()) ? '<a href="' . get_permalink() . '#comments" rel="nofollow">' . thesis_num_comments($num_comments, true) . '</a>' : apply_filters('thesis_comments_link_closed', __('Comments not allowed', 'thesis')); #wp #filter
...
function thesis_comments_intro($number, $open, $type = 'comments') {
	...
	if ($open && $type == 'comments') {
		if ($number == 0)
			$add_link = '&#8230; <a href="#respond" rel="nofollow">' . __('Be the first to comment!', 'thesis') . '</a>';
		elseif ($number == 1)
			$add_link = '&#8230; ' . __('', 'thesis') . '<a href="#respond" rel="nofollow">' . __('Participate in the discussion', 'thesis') . '</a>';
		elseif ($number > 1)
			$add_link = ' ' . __('', 'thesis') . '<a href="#respond" rel="nofollow">' . __('Participate in the discussion', 'thesis') . '</a>';
	}
	else
